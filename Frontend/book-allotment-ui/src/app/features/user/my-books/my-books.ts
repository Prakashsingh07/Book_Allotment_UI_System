  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-purple-950 p-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      ...
      <div class="bg-gradient-to-br from-rose-600/30 to-rose-700/30 backdrop-blur-sm rounded-2xl border border-rose-500/20 p-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-white">{{ overdueCount() }}</p>
        </div>
      </div>
    </div>
    ...
    <div *ngIf="!loading && logs.length > 0"
      ...
      <div *ngFor="let log of logs"
           ...
           [ngClass]="isOverdue(log)
             ? 'border-rose-500/40 hover:border-rose-400/60 hover:shadow-2xl hover:shadow-rose-500/20'
             : 'border-white/10 hover:border-indigo-400/50 hover:shadow-2xl hover:shadow-indigo-500/20'">
        ...
        <div class="relative h-52 overflow-hidden flex-shrink-0">
          ...
          <span class="absolute top-3 right-3 text-xs font-bold px-3 py-1 rounded-full shadow-lg backdrop-blur-sm border"
                [ngClass]="{
                  'bg-emerald-500/80 text-white border-emerald-400/30': log.status === 'Allotted' && !isOverdue(log),
                  'bg-rose-500/80 text-white border-rose-400/30':       isOverdue(log),
                  'bg-blue-500/80 text-white border-blue-400/30':       log.status === 'Returned',
                  'bg-gray-500/80 text-white border-gray-400/30':       log.status === 'Revoked'
                }">
            <ng-container *ngIf="log.status === 'Allotted' && !isOverdue(log)">✓ Issued</ng-container>
            <ng-container *ngIf="isOverdue(log)">⚠ Overdue</ng-container>
            <ng-container *ngIf="log.status === 'Returned'">↩ Returned</ng-container>
            <ng-container *ngIf="log.status === 'Revoked'">✗ Revoked</ng-container>
          </span>

          <!-- Overdue fine ticker on the card -->
          <div *ngIf="isOverdue(log)"
               class="absolute bottom-3 left-3 right-3 bg-black/60 backdrop-blur-sm
                      rounded-xl px-3 py-1.5 flex items-center justify-between">
            <span class="text-rose-300 text-xs font-semibold">Fine accruing</span>
            <span class="text-rose-400 font-extrabold text-sm">₹{{ getLiveFine(log) }}</span>
          </div>
        </div>
        ...
        <div class="p-4 flex flex-col flex-1 gap-3">
          ...
          <!-- Due date info -->
          <p *ngIf="log.status === 'Allotted' && log.dueDate" class="text-xs"
             [ngClass]="isOverdue(log) ? 'text-rose-400' : 'text-white/40'">
            {{ isOverdue(log) ? '⏰ Was due ' : '📅 Due ' }}{{ log.dueDate | date:'mediumDate' }}
          </p>

          <div class="mt-auto">

            <!-- ── Overdue + fine NOT paid → must pay first ── -->
            <ng-container *ngIf="isOverdue(log) && !log.finePaid">
              <a routerLink="/user/my-activity"
                ...
                Pay Fine First (₹{{ getLiveFine(log) }})
              </a>
              <p class="text-rose-400/60 text-xs text-center mt-1.5">Pay the fine before returning</p>
            </ng-container>

            <!-- ── Overdue + fine PAID → can return now ── -->
            <ng-container *ngIf="isOverdue(log) && log.finePaid">
              <button (click)="returnBook(log.id)"
                      class="w-full py-2.5 rounded-xl text-sm font-bold transition-all duration-200
              ...
              <p class="text-emerald-400/60 text-xs text-center mt-1.5">✓ Fine paid — ready to return</p>
            </ng-container>

            <!-- ── Active (not overdue) → return normally ── -->
            <button *ngIf="log.status === 'Allotted' && !isOverdue(log)"
...
export class MyBooksComponent implements OnInit {
  ...
  isOverdue(log: any): boolean {
    if (log.status !== 'Allotted') return false;
    const dueMs = this.parseDueMs(log.dueDate);
    return dueMs > 0 && this.nowMs > dueMs;
  }

  getLiveFine(log: any): number {
    const dueMs  = this.parseDueMs(log.dueDate);
    if (!dueMs)  return 0;
  ...
  overdueCount(): number {
    return this.logs.filter(l => this.isOverdue(l)).length;
  }
  ...
  returnBook(allotmentId: number) {
    ...
    const log = this.logs.find(l => l.id === allotmentId);
    if (log && this.isOverdue(log) && !log.finePaid) {
      this.showToast('Please pay the fine before returning this book.', false);
      return;
    }
    ...
    this.requestService.returnBook(allotmentId).subscribe({
      next: () => {
        this.logs = this.logs.map(l =>
          l.id === allotmentId ? { ...l, status: 'Returned' } : l
        );
        this.showToast('Book returned successfully!', true);
        this.cdr.markForCheck();
      },
      error: (err: any) => {
        this.showToast(err?.error?.message || 'Return failed. Please try again.', false);
        this.cdr.markForCheck();
      }
    });
  }
  ...
  showToast(message: string, success: boolean) {
    ...
    this.cdr.markForCheck();
    setTimeout(() => { this.toast.show = false; this.cdr.markForCheck(); }, 4000);
  }
}
